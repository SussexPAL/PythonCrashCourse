...
In OO design, inheritance means much the same thing. One class can inherit certain features from another. Let's take a look
...
class PalCar(Car):
```
This is the Syntax python uses for inheritance. we'll break it down line by line. First, the opening line of the class definition, with these brackets that we haven't seen before. It is in these brackets we put the class we want to inherit from.
...
From here we can add new features to PalCar, like methods and properties. Let's add a method for beeping the car horn!
``` py
class PalCar(Car):
    def horn(self):
        print("BEEEEP!!")
...
```
We'll see that we can access any method or property of the superclass from the subclass, but if we try and access anything specific to PalCar from the Car superclass, we get an error!
``` py
...
completely re-writing a an overridden method is good if you want to completely change how it works, but here it seems kind of unnecessary: we're just changing one thing after all. Remember what we've said before about wanting to reduce redundancy in our code: "say something once, why say it again". Luckily there is a solution.

In python, we can use a special method called `super()`. Super allows an subclass to access any attributes of it's superclass. We do this by calling whichever method or property we want from a super call, like this:
...
We could define flying cars with no wheels, self driving cars, a "Super PalCar" which you never need to refuel (somehow).
``` py
class FlyingCar(Car):
    def __init__(self, num_plate: str, colour: str):
...
Likely because Python is one of the few languages which supports it, and also because it can get pretty messy, multiple inheritance isn't very common. It's not likely to be something you'll need to use in many cases, but it does have a few super interesting niche use cases, which could be worth having a play around with once you've gotten comfortable with the fundamentals.
