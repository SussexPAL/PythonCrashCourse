...
``` 
I say "the right number of", because we can define functions was as many parameters as we want, that is; none, one, or many. If we have more than one parameter, just need to make sure they have different names, and are comma separated. Just remember that the order is important.
```py
def add(num1, num2):
    num3 = num1 + num2
...
```
Once a function hits a return statement it will exit out of the call with whatever value we've given it. This value can be a variable, and expression, even another function call - here we're giving it a literal boolean. Now, depending on what value x is passed to it. - is_even will evaluate to either True or False.
...
[CUT AWAY]
Recall all the way back to learning about expressions - we said how any expression, no matter how long or convoluted it looked to us, would AT RUNTIME (that is to say, when the program runs) be evaluated down to a single value.
```py
exp1 = (1 + 0.5 + 1.5) * 2 / 3 + (5 ** 2 % 4) # This is literally just 3.0
print(exp1)
# See?
```
Functions work in much the same way, where the value that the function call is ultimately evaluated to at runtime, is the value that the function returns.
``` py
...
[CENTER CAM]
In computer science, we like to generalize functions as black box. That is, when we call a function, we put values in, and a value comes out again. Once we have that final value in return we can no longer know, nor should we really care, about exactly how the function inside in order to reach that value - so long as it does the job.
...
With this, we've covered all the fundamentals we need to know about functions. Now would be a great time to do some exercises, to see how they work for yourselves, and hopefully your understanding as much as possible
