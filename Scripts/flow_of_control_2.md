...
Theres a simple question we've yet to address - one which would have raised itself before we even wrote our first line of code all the way back in 'algorithmic thinking'. Thinking of code as instructions: what if we want our instructions to repeat themselves? IE "do this until this", "repeat 5 times" and so on. Is there a way we can do this in Python.
...
[CENTER CAM]
This raises an interesting question: can we get stuck in a while loop.

Unfortunately we can - and it's something we want to avoid. With that example we knew what we were doing: we knew we were starting at 10, we remembered to decrease it by 1 each time, so we knew eventually we'd reach zero and exit out of the loop. Depending on what we're trying to do, what condition we're trying to meet, and crucially what happens INSIDE the loop each time, we may not be so lucky. This is why we need to take care when using while loops - otherwise our program might get stuck.

*[Optional bit here but I appreciate I'm info dumping a bit]*
[SIDE CAM]
We can even, if we're feeling mischievous, do it deliberately just to see what happens. Just for fun, here's an example of the infamous 'while true' loop.
``` py
while True:
...
for item in iterable:
    print(item)
```
For loops rely on some collection of something to work - in the context of a for loop we call this an iterable, that is something that can give us multiple elements to iterate through. The for loop will run once for each of those elements. Remember our collection data types like lists - these are examples of iterables. let's see this in action.
...
[CENTER CAM]
The name of the loop variable doesn't actually matter too much. Some common names used by convention include "item", "element", and "i" - but so long as it's a valid variable name, and doesn't conflict with anything else, you can pick whatever. Just like any other variable, though, its advisable to pick a name that makes sense to you as your writing it, and ideally will still make sence when you come back to it after not having looked it for a month...!

For the above example *[put back on screen if theres room]*, "member", "name" or even "person" would have all been perfectly sensible choices, but maybe not "x", or "thing", or "ftftft".
...
Computer Scientists often like food metaphors, so, we use the metaphor that it makes the code "sweeter".
